    ...
    try {
        ...
        if(authMode !== AUTH_MODE.MC_REFRESH) {
            ...
            if(accessTokenResponse.responseStatus === RestResponseStatus.ERROR) {
                const errDetail = accessTokenResponse.microsoftErrorCode === MicrosoftErrorCode.UNKNOWN && accessTokenResponse.error
                    ? (accessTokenResponse.error?.message || accessTokenResponse.error?.code || String(accessTokenResponse.error))
                    : null
                return Promise.reject(microsoftErrorDisplayable(accessTokenResponse.microsoftErrorCode, errDetail))
            }
        ...
        if(xblResponse.responseStatus === RestResponseStatus.ERROR) {
            const errDetail = xblResponse.microsoftErrorCode === MicrosoftErrorCode.UNKNOWN && xblResponse.error
                ? (xblResponse.error?.message || xblResponse.error?.code || String(xblResponse.error))
                : null
            return Promise.reject(microsoftErrorDisplayable(xblResponse.microsoftErrorCode, errDetail))
        }
        const xstsResonse = await MicrosoftAuth.getXSTSToken(xblResponse.data)
        if(xstsResonse.responseStatus === RestResponseStatus.ERROR) {
            const errDetail = xstsResonse.microsoftErrorCode === MicrosoftErrorCode.UNKNOWN && xstsResonse.error
                ? (xstsResonse.error?.message || xstsResonse.error?.code || String(xstsResonse.error))
                : null
            return Promise.reject(microsoftErrorDisplayable(xstsResonse.microsoftErrorCode, errDetail))
        }
        const mcTokenResponse = await MicrosoftAuth.getMCAccessToken(xstsResonse.data)
        if(mcTokenResponse.responseStatus === RestResponseStatus.ERROR) {
            const errDetail = mcTokenResponse.microsoftErrorCode === MicrosoftErrorCode.UNKNOWN && mcTokenResponse.error
                ? (mcTokenResponse.error?.message || mcTokenResponse.error?.code || String(mcTokenResponse.error))
                : null
            return Promise.reject(microsoftErrorDisplayable(mcTokenResponse.microsoftErrorCode, errDetail))
        }
        const mcProfileResponse = await MicrosoftAuth.getMCProfile(mcTokenResponse.data.access_token)
        if(mcProfileResponse.responseStatus === RestResponseStatus.ERROR) {
            const errDetail = mcProfileResponse.microsoftErrorCode === MicrosoftErrorCode.UNKNOWN && mcProfileResponse.error
                ? (mcProfileResponse.error?.message || mcProfileResponse.error?.code || String(mcProfileResponse.error))
                : null
            return Promise.reject(microsoftErrorDisplayable(mcProfileResponse.microsoftErrorCode, errDetail))
        }
        return {
            accessToken,
            accessTokenRaw,
        ...
        log.error('Microsoft auth flow error:', err)
        const errMsg = err?.message || err?.code || String(err)
        return Promise.reject(microsoftErrorDisplayable(MicrosoftErrorCode.UNKNOWN, errMsg))
    }
}
