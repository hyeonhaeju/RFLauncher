        ...
        if (Object.prototype.hasOwnProperty.call(queryMap, 'error')) {
            ...
            AuthManager.addMicrosoftAccount(authCode).then(value => {
                ...
                .catch((displayableError) => {
                    ...
                    let desc = actualDisplayableError.desc || ''
                    if (actualDisplayableError.detail) {
                        desc += '\n\n상세: ' + actualDisplayableError.detail
                    }

                    switchView(getCurrentView(), viewOnClose, 500, 500, () => {
                        setOverlayContent(actualDisplayableError.title, desc, Lang.queryJS('login.tryAgain'))
                        setOverlayHandler(() => {
                            toggleOverlay(false)
                        })
                        toggleOverlay(true)
...
function populateAuthAccounts(){
        ...
        const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    ...
                    <button class="settingsAuthAccountSkinEdit">${Lang.queryJS('settings.authAccountPopulate.skinEdit')}</button>
                </div>
            </div>
        </div>`
    ...
    // 스킨 변경 버튼 클릭 이벤트
    Array.from(document.getElementsByClassName('settingsAuthAccountSkinEdit')).forEach(btn => {
        btn.addEventListener('click', () => {
            shell.openExternal('https://www.minecraft.net/ko-kr/msaprofile/mygames/editskin')
        })
    })
}
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/hyeonhaeju/RFLauncher/releases.atom',
        success: (data) => {
